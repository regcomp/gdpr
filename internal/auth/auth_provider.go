...
func CreateAuthProvider(cfg *config.AuthProviderConfig, secrets *secrets.AuthProviderSecrets) (IAuthProvider, error) {
	...
	case MockProviderType:
		return createLocalAuthProvider(), nil
	default:
...
func createLocalAuthProvider() *LocalAuthProvider {
	mp := &LocalAuthProvider{}
...
func (mp *LocalAuthProvider) generateJWTWithClaims(audience, issuer string, tokenDuration time.Duration) (string, error) {
...
func (mp *LocalAuthProvider) GetProviderType() string {
	return MockProviderType
}

func (mp *LocalAuthProvider) AuthenticateUser(w http.ResponseWriter, r *http.Request, callback *url.URL) {
...
func (mp *LocalAuthProvider) ValidateAccessToken(token string) (*CustomClaims, error) {
	return verifyJWTWithClaims(token, &CustomClaims{}, mp.publicKey)
}

func (mp *LocalAuthProvider) GetNewAccessToken(refreshToken string, r *http.Request) (string, error) {
